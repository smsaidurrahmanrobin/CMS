    ...
    if(isset($_POST['email'])){
        ...
        if(email_exists($email)){
          ...
          $stmt = mysqli_prepare($connection, "UPDATE users SET token ='{$token}' WHERE user_email= ? ");
            ...
            $mail->CharSet = 'UTF-8';
            $mail->setFrom('edwindiaz@faculty.com', 'edwin diaz');
            $mail->addAddress($email);
            
            $mail->Subject = 'This is a test email';
            $mail->Body = '<h1>Please click to reset your password <a href="http://localhost:80/cms/reset.php?email='.$email. '&token='.$token.'">"http://localhost:80/cms/reset.php?email='.$email. '&token='.$token.'"</a></h1>';
